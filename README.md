...
- ✅ 自动处理多张题目图片
- ✅ **智能图片压缩**：上传前自动压缩到 800KB 以下，避免请求过大
- ✅ **长超时设置**：API 连接超时 120 秒，适应大请求
...
- ✅ MCP服务器架构，可集成到其他工具
- ✅ 结果按题目独立保存，便于管理
- ✅ **详细日志记录**：所有操作记录到 run.log 文件，便于调试

## 性能优化

### 图片压缩
系统会自动压缩上传的图片，减少 API 请求大小：
- **目标大小**: 800 KB 以下
- **压缩策略**:
  1. 调整 JPEG 质量（85% → 25%）
  2. 如需要，缩小图片尺寸
  3. 保持最佳视觉效果
- **日志记录**: 显示原始大小、压缩率、最终大小
- 详细说明请查看 `IMAGE_COMPRESSION.md`

### 连接超时
- **API 超时**: 120 秒（默认 60 秒）
- **适用场景**: 处理大图片或复杂题目
- **错误处理**: 超时会自动记录错误日志
